...
<x-app-layout title="Editar consentimiento">
    <div class="max-w-3xl">
        <h1 class="text-lg font-medium mb-1">Editar consentimiento</h1>
        <p class="text-sm text-aura-gray mb-6">{{ $patient->full_name }} · Borrador</p>

        <form method="POST" action="{{ route('consents.update', $consent) }}" class="space-y-6">
            @csrf
            @method('PUT')
