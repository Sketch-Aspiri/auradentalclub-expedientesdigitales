...
- **Listado de pacientes en Livewire** (2026-08-27): búsqueda en vivo, toggle activos/archivados y restaurar sin recargar; ruta `patients.index` conservada vía wrapper controller. Añadida columna "Último doctor" y pantalla global de odontograma (`/odontogram`).
- **Perfil de la cuenta** (2026-08-27, no estaba en ningún sprint — extensión de auth del Sprint 0): pantalla `/profile` para editar nombre / correo (con contraseña actual) / contraseña, menú desplegable en la barra superior, y **foto de perfil** por usuario (disco privado, `route('profile.photo')` solo-propia, re-codificada a JPEG). Reemplaza el monograma "a" compartido. Política de contraseñas subida a mín. 12 en todo el proyecto. Revisado por `ecc:security-reviewer` (WARNING, 0 críticos) y `code-reviewer`. Hardening diferido a Sprint 8, ver ahí.
