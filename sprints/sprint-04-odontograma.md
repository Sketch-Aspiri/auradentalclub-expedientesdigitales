...
| 2026-08-27 | Pantalla global del odontograma (`GET /odontogram`, enlazada desde el dashboard y la navegación): componente `App\Livewire\Odontogram\Browser` con buscador de pacientes que embebe el odontograma del paciente elegido; `?paciente=` para enlace directo; el acceso por ficha (`patients.odontogram`) se mantiene. `OdontogramController` pasa de invokable a `index`/`show`. | Implementación + `code-reviewer` | Veredicto BLOQUEADO → CRÍTICO corregido: `patientId` pública sin `#[Locked]` permitía cargar el odontograma de cualquier paciente sin re-autorizar ni auditar; se añadió `#[Locked]` + `authorize('viewAny')` en las computed + test del vector. +12 tests (`OdontogramBrowserTest`). Suite en 180 verde. |
