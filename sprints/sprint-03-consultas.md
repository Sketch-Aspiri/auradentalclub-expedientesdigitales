...
- **[BAJO] `ConsultationPolicy::restore()`** existe pero el trait `Auditable` no engancha el evento `restored` — sin ruta/UI de restauración hoy, a resolver cuando se implemente (requiere ampliar el enum `action` de `audit_logs`). Mismo pendiente para `PatientPolicy` / `MedicalHistoryPolicy`.
- **Ampliar `Patient::forceDeleting`** al añadir los módulos de consentimientos, archivos, hoja de evolución y odontograma (cada nueva relación clínica debe purgarse ahí para conservar el rastro de auditoría).
